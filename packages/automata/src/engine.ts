import { Automaton, StateNode, TransitionEdge, AnalysisResult, ValidationIssue, SimulationResult, SimulationStep, GeneratedTestSequence, NFAToDFAResult, MinimizationResult } from './types.js';

export function analyzeAutomaton(automaton: Automaton): AnalysisResult {
  ...
  const canReachFinal = new Set<string>(finalStateIds);
  let changed = true;
  while (changed) {
    changed = false;
    for (const t of transitions) {
...
export function simulateAutomaton(automaton: Automaton, events: string[]): SimulationResult {
...
export function generateTestSequences(automaton: Automaton, maxTests = 20, maxPathLength = 25): GeneratedTestSequence[] {
  const initial = automaton.states.find(s => s.isInitial);
  if (!initial) return [];

  const sequences: GeneratedTestSequence[] = [];
  const finalStates = automaton.states.filter(s => s.isFinal);

  // Shortest path BFS to final states
  for (const finalState of finalStates) {
    if (sequences.length >= maxTests) break;

    const queue: { stateId: string; events: string[] }[] = [{ stateId: initial.id, events: [] }];
    ...
    while (queue.length > 0) {
      ...
      if (events.length >= maxPathLength) continue;
...
export function convertNFAToDFA(nfa: Automaton): NFAToDFAResult {
  ...
  function getEpsilonClosure(stateIds: string[]): string[] {
    const closure = new Set<string>(stateIds);
    const queue = [...stateIds];
    while (queue.length > 0) {
      const curr = queue.shift()!;
      const epsilonTransitions = nfa.transitions.filter(
        t => t.fromStateId === curr && (t.event === '' || t.event.toLowerCase() === 'ε' || t.event.toLowerCase() === 'eps')
      );
  ...
  const dfaSubsets: string[][] = [initialClosure];
  const dfaStateMap = new Map<string, string>(); // subsetKey -> dfaStateId
  
  const dfaStates: StateNode[] = [];
  const dfaTransitions: TransitionEdge[] = [];
  const subsetMap: { dfaStateName: string; nfaStateIds: string[] }[] = [];

  function getSubsetKey(subset: string[]): string {
    return subset.join(',');
  }

  function getOrCreateDFAState(subset: string[]): string {
  ...
  while (queue.length > 0) {
    ...
    for (const symbol of alphabet) {
      ...
      const targetNFAIds = new Set<string>();
...
export function minimizeDFA(dfa: Automaton): MinimizationResult {
  ...
  if (finalStateIds.size > 0) partitions.push(finalStateIds);
  if (nonFinalStateIds.size > 0) partitions.push(nonFinalStateIds);

  const alphabet = Array.from(new Set(dfa.transitions.map(t => t.event)));

  function getPartitionIndex(stateId: string, parts: Set<string>[]): number {
  ...
  while (changed) {
    ...
    for (const group of partitions) {
      ...
      for (const stateId of group) {
        ...
        const existing = subGroups.get(signature) || new Set<string>();
  ...
  const oldToNewStateId = new Map<string, string>();
  const mergedGroups: { mergedName: string; originalStateIds: string[] }[] = [];
